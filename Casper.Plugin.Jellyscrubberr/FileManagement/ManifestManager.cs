...
public class ManifestManager
{
    ...
    public bool ManifestMatches(BaseItem item)
    {
        ...
        try
        {
            ...
            // check if any of the manifest values are null (they dont exist in the manifest file). This happens when the manifest file is either old or not generated by Jellyscrubberr
            if (manifest.Version == null || manifest.imageWidthResolution == null || manifest.qScaleInput == null || manifest.imageInterval == null)
            {
                if (_config.shouldRegenerateIfOldManifest == true)
                {
                    return false;
                }
                else
                {
                    _logger.LogWarning("Regeneration of manifest is disabled, skipping manifest generation for: {0}", item.Name);
                    return true;
                }
            }
